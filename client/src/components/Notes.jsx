...
import { Save, ArrowLeft, Clock, CheckCircle2, Activity, Trash2 } from 'lucide-react';
...
const NoteEditor = () => {
  ...
  // When the drawer opens, check if there's an unsaved draft in localStorage
  useEffect(() => {
    const draftTitle = localStorage.getItem('networkNoteDraftTitle');
    const draftContent = localStorage.getItem('networkNoteDraftContent');
    
    if (draftTitle) setTitle(draftTitle);
    if (draftContent) setContent(draftContent);
  }, []); // Empty dependency array means this runs only once when opened

  // 2. AUTO-SAVE DRAFT TO LOCAL STORAGE
  // Whenever the user types, update localStorage so it survives drawer closes
  useEffect(() => {
    // Prevent overriding localStorage with empty strings on initial mount if there is a draft
    if (title !== '' || content !== '') {
      localStorage.setItem('networkNoteDraftTitle', title);
      localStorage.setItem('networkNoteDraftContent', content);
      setSaveStatus('Unsaved draft');
    }
  }, [content, title]);
  ...
  const saveNoteToDatabase = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        // 3. CLEAR DRAFT AFTER SUCCESSFUL DB SAVE
        localStorage.removeItem('networkNoteDraftTitle');
        localStorage.removeItem('networkNoteDraftContent');
        setTitle('');
        setContent('');
        
        // Optional: Alert the user it was successful
  ...
  // Optional: A function to manually trash the current draft
  const clearDraft = () => {
    ...
    <div className="min-h-screen bg-[#0B1121] flex flex-col font-sans text-slate-300 selection:bg-indigo-500/30">
      ...
      <header className="flex items-center justify-between px-4 md:px-8 py-4 border-b border-slate-800/60 bg-[#0B1121]/90 backdrop-blur-md sticky top-0 z-30">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          {/* Added a Trash button so users can easily clear a draft if they change their mind */}
          {(title || content) && (
            <button 
              onClick={clearDraft}
        ...
        <div className="w-full max-w-5xl mx-auto px-4 sm:px-8 py-8 md:py-12 flex flex-col flex-1">
          ...
          <div className="enterprise-quill flex-1 flex flex-col">
            <ReactQuill
              ...
              onChange={setContent} 
              modules={modules}
              placeholder="Start typing your incident observations here..."
            />
          </div>
