...
export default function DashBoard() {
    ...
    return (
        <div className="w-full min-h-screen bg-[#06121f] text-white p-10 flex flex-col items-center justify-center selection:bg-[#19d5ff33] relative overflow-hidden">
            ...
            {/* Adjusted grid layout to accommodate 5 items gracefully */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8 max-w-[90rem] w-full relative z-10">
                ...
                {/* NEW: Incident Logs (Notes) */}
                <Link href={'/api/notes'} prefetch={false}>
                    <motion.div 
                        whileHover={{ y: -10 }} 
                        className={`bg-[#0b1f33] border p-10 rounded-[2.5rem] flex flex-col items-center justify-center text-center group transition-all duration-500 h-[350px] shadow-2xl ${activeBorder}`}
                    >
                        <div className="mb-6 relative transition-transform group-hover:scale-110 duration-500">
                            <Image src={"/note.png"} width={100} height={100} alt="Incident Logs" className="opacity-80 group-hover:opacity-100" />
                        </div>
                        <h3 className="text-xl font-black uppercase italic tracking-tighter mb-2 group-hover:text-[#19d5ff]">Incident Logs</h3>
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Network Documentation</p>
                    </motion.div>
                </Link>
